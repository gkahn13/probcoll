...
class ProbcollModelPointquad(ProbcollModel):
    ...
    def _get_old_graph_inference(self, graph_type='fc'):
        ...
        if graph_type == 'fc':
            ...
            return OldProbcollModel._graph_inference_rnn
        else:
        ...
        with tf.name_scope(name + '_inference'):
                    ...
                    with tf.name_scope('hidden_{0}_b{1}'.format(i, b)):
                        ...
                        if dropout is not None:
                            ...
                            if name == 'eval':
                                ...
                                layer = tf.multiplytiply(layer, dp)
            ...
            with tf.name_scope('combine_bootstraps'):
                ...
                output_pred_std = tf.sqrt(std_normalize * tf.add_n(
                    [tf.square(tf.subtract(output_pred_b, output_pred_mean)) for output_pred_b in bootstrap_output_preds]))
                ...
                output_mat_std = tf.sqrt(std_normalize * tf.add_n(
                    [tf.square(tf.subtract(output_mat_b, output_mat_mean)) for output_mat_b in bootstrap_output_mats]))
    ...
    def _graph_inference_rnn(name, T, bootstrap_X_inputs, bootstrap_U_inputs, bootstrap_O_inputs,
        ...
        with tf.name_scope(name + '_inference'):
            ...
            with tf.name_scope('combine_bootstraps'):
                ...
                std_normalize = (1 / float(num_bootstrap - 1)) if num_bootstrap > 1 else 1
                output_pred_std = tf.sqrt(std_normalize * tf.add_n(
                    [tf.square(tf.subtract(output_pred_b, output_pred_mean)) for output_pred_b in
                     bootstrap_output_preds]))
    ...
    def _create_multistep_graph(dX, dU, dO, T, num_bootstrap, dropout,
        ...
        with graph.as_default():
            ...
            std_normalize = (1 / float(num_bootstrap - 1)) if num_bootstrap > 1 else 1
            output_pred_std = tf.sqrt(std_normalize * tf.add_n(
                [tf.square(tf.subtract(output_pred_b, output_pred_mean)) for output_pred_b in output_preds]))
            # output_pred_std = tf.sqrt(std_normalize * tf.add_n(
            #     [tf.square(tf.sigmoid(tf.subtract(output_mat_b, output_mat_mean))) for output_mat_b in output_mats])) # TODO chug through nn or not?
