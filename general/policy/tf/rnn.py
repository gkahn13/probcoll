...
from general.policy.tf import rnn_cell
...
def sample_dropout_mask(shape, keep_prob):
    assert (type(keep_prob) is float and 0 < keep_prob and keep_prob <= 1.0)
    return (1 / keep_prob) * tf.select(tf.random_uniform(shape) > 1 - keep_prob, tf.ones(shape), tf.zeros(shape))

def dynamic_rnn(cell, inputs, sequence_length=None, initial_state=None,
                dtype=None, parallel_iterations=None, swap_memory=False,
                time_major=False, scope=None, dropout=None): # TODO add dropout
...
def _dynamic_rnn_loop(cell,
  ...
  def _time_step(time, output_ta_t, state, dropout=None, dropout_mask=None): # TODO: pass in dropout mask
    ...
    Args:
      ...
      dropout: keep_prob
      dropout_mask: Bernoulli random vector applied to internal state (or not if None)
    ...
    call_cell = lambda: cell(input_t, state, dropout=dropout, dropout_mask=dropout_mask) # TODO: pass in dropout mask
    ...
    return (time + 1, output_ta_t, new_state, dropout, dropout_mask) # TODO: pass in dropout mask

  dropout_mask = None
  if dropout is not None: # TODO added
    shape = [batch_size, cell.state_size]
    dropout_mask = sample_dropout_mask(shape, dropout)

  _, output_final_ta, final_state, _, _ = control_flow_ops.while_loop( # TODO: returns dropout at end
      cond=lambda time, *_: time < time_steps,
      body=_time_step,
      loop_vars=(time, output_ta, state, dropout, dropout_mask), # TODO: pass in dropout
      parallel_iterations=parallel_iterations,
      swap_memory=swap_memory)
