...
class AnalyzePointquad(Analyze):
    ...
    def __init__(self, on_replay=False):
        ...
        self.agent = AgentPointquad(self.world, self.dynamics, dyn_noise=params['prediction']['dagger']['dyn_noise'])
    ...
    def _evaluate_probcoll_model(self):
        ...
        while True:
            ...
            for pose, radius, height in zip(wd['cyl_poses'], wd['cyl_radii'], wd['cyl_heights']):
                self.world.env.add_cylinder(pose, radius, height)
            ...
            for pos in positions:
                ...
                s.set_O(self.agent.get_observation(s.get_X(t=0), noise=False), t=slice(0, bootstrap.T))
        ...
        while True:
            ...
            for i, xpos in enumerate(np.linspace(-0.5, 1.5, dx)):
                for j, ypos in enumerate(np.linspace(0.6, -0.6, dy)):
                    ...
                    x0 = s.get_X(t=0)
                    rollouts_ij = [self.agent.sample_policy(x0, p, noise=ZeroNoise(params), T=bootstrap.T)
